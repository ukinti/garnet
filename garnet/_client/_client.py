...
from garnet.helpers import ctx


class TelegramClient(_TelethonTelegramClient, ctx.ContextInstanceMixin):
    __is_telegram_garnet__ = True
    __dispatch_hook__ = None

    def __delegate_built_event_to(
        self, where: Callable[["EventBuilderDict"], Awaitable[None]], /
    ):
        self.__dispatch_hook__ = where
