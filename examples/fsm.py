...
from pomegranate import TelegramClient, FSMContext, MessageText, CurrentState
...
@bot.on(CurrentState.exact("stateName"))
async def name_handler(update: custom.Message, context: FSMContext):
    await context.set_data({"name": update.raw_text})
    await update.reply(f"{update.raw_text}, please enter your age:", buttons=custom.Button.force_reply())
    await context.set_state("stateAge")


@bot.on(CurrentState.exact("stateAge"), MessageText.isdigit())
...
@bot.on(CurrentState.exact("stateAge"), not MessageText.isdigit())
async def age_incorrect_handler(update: custom.Message):
    await update.reply(f"Please try again! Age must be digit :D")


@bot.on(CurrentState.equal("stateGender"), MessageText.between("Abuser", "Dishwasher", "Cat"))
